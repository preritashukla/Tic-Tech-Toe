...
async def test_master_repo():
    print("Test 1: No repository provided (Should Ask User)")
    try:
        res = await handle_github_tool("get_repository", {})
        print(f"❌ Failed: Automatically succeeded: {res.get('repo_full_name')}")
    except Exception as e:
        print(f"✅ Correctly Failed with message: {e}")
    ...
    try:
        await handle_github_tool("get_repository", {"repo": "google/repo"})
