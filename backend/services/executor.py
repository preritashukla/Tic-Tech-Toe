    ...
    async def _execute_fallback_node(
        ...
        for attempt in range(1, node.retry.max_attempts + 1):
            try:
                ...
                elif node.tool == "sheets":
                    from services.integrations.sheets_integration import execute_sheets
                    result = await execute_sheets(node.action, resolved_params, getattr(self.context, '__dict__', {}))
                    if result.get("status") == "error":
                        raise Exception(result.get("error"))
                    output = result.get("output", {})
                elif node.tool == "github":
                    # For GitHub, import the real github_mcp handler and use it natively
                    import sys
                    from agentic_mcp_gateway.github_mcp import handle_github_tool
                    # handle_github_tool doesn't use standard status/output wrapping, it just returns data
